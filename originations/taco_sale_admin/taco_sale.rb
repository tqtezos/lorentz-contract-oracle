...
class Sale
  ...
  def self.new_user(name, address)
    { 'user' => {
      'name' => name,
      'address' => address } }
  end
  ...
  def self.sale_name(held, wanted, held_price, wanted_price)
    "Sale#{self.token_name(held)}_#{self.token_name(wanted)}_#{held_price}_#{wanted_price}"
  end
  ...
  { 'originate' => {
      ...
      'initial_storage' => {
        'admin' => "$#{self.admin}",
        'held' => "$#{self.token_name(held)}",
        'wallet' => "$#{self.admin}",
        'wanted' => "$#{self.token_name(wanted)}",
  ...
  def self.add_new_token(prng, new_token, token_address)
    base_token = prng.rand(16..31)
    Enumerator.new do |y|
      y << self.new_user(self.token_name(new_token), token_address)
      y << self.originate_sale(base_token, new_token, 1, 1)
      y << self.approve(self.token_name(base_token), self.sale_name(base_token, new_token, 1, 1), 100)
    end
  end
end
...
# (ruby taco_sale.rb add_sale taco_sale_user.originated.yaml | sed 's/"\(\$.*\)"/\1/') > example.yaml
if ARGV[0] == 'add_sale'
  yaml = YAML.load(File.read(ARGV[1]))
  unless yaml.has_key?('originated')
    raise "no originated tokens"
  end
  yaml_originated = yaml['originated']
  unless yaml_originated.length == 1
    raise "expected a single originated contract"
  end
  originated_name = yaml_originated.to_a[0][0]
  originated_address = yaml_originated.to_a[0][1]['address']
  puts File.read("#{__dir__}/taco_sale_config_template.yaml")
  puts(YAML.dump(YAML.load(File.read("#{__dir__}/../taco_sale/taco_sale.originated.yaml"))['originated'].to_a.map do |x,y|
    {'user'=>{'name'=>x,'address'=>y['address']}}
  end).lines.drop(1).map{|x|'  '+x}.join)
  puts
  puts YAML.dump({ 'do' => Sale.add_new_token(prng, originated_name, originated_address).to_a }).lines.map{|x|"  "+x}.drop(2)
elsif ARGV[0] == 'parse_sale'
  yaml = YAML.load(File.read(ARGV[1]))
  unless yaml.has_key?('originated')
    raise "no originated tokens"
  end
  ...
  originated_name = yaml_originated.to_a[0][0]
  originated_address = yaml_originated.to_a[0][1]['address']
  puts YAML.dump([{'user'=>{'name' => originated_name,'address' => originated_address}}]).lines.drop(1).map{|x|'  '+x}.join
else
  raise "unrecognized command"
end

# command to generate .yaml
# ruby taco_sale.rb 'Alice' 'KT1KT1KT1KT1KT1KT1KT1KT1KT1' | sed 's/"\(\$.*\)"/\1/' | sed '3d' | sed 's/^[^d<]/  &/' 
